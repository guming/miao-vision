<script lang="ts">
   ...
   *
   * Displays time-series data with filled areas under lines.
   * Supports multiple series, stacking, and customization.
   */
  ...
  let showLine = $derived(config.showLine !== false)
  let fillOpacity = $derived(config.fillOpacity ?? 0.7)
...
<div class="area-chart-container {config.class || ''}">
  {#if title}
    ...
    <div class="chart-legend">
      {#each series as s}
        <div class="legend-item">
          <span class="legend-area" style="background-color: {s.color}; opacity: {fillOpacity}"></span>
  ...
  >
    <g transform="translate({margin.left}, {margin.top})">
      ...
      <!-- Area series (render in reverse for correct stacking) -->
      {#each series.slice().reverse() as s}
        <!-- Filled area -->
        <path
          d={s.areaPath}
          fill={s.color}
